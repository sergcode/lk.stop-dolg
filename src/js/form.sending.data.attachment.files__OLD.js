...
window.addEventListener('DOMContentLoaded', () => {
	const mediaQueryTaskSmart = window.matchMedia('(max-width: 440.98px)'),
				mediaQueryTaskTable = window.matchMedia('(max-width: 854px) and (orientation: landscape)');
	...
	function deleteAttachedFiles(wrapperCreatedFiles, inputTypeFiles) {
		if (inputTypeFiles && wrapperCreatedFiles && wrapperCreatedFiles.childElementCount > 0) {
			const parent = wrapperCreatedFiles.closest('.toggle-modal.active'),
						wrapperBtnDeleteFiles = inputTypeFiles.closest('.attach__search_wrapper-created-files').querySelector('.attach__delete-files');
	...
	function deleteAttachedFile(wrapperCreatedFiles) {
		const parent = wrapperCreatedFiles.closest('.toggle-modal.active'),
					btnDel = wrapperCreatedFiles.querySelectorAll('.attach__delete');
