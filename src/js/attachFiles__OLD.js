$(document).ready(function () {
  $('.attach.attach-files').each(function() { // на случай, если таких групп файлов будет больше одной
    var attach = $(this),
    ...
    var newButtonAttachFiles = '<div class="attach__item"><label class="attach__button"><span class="attach__up button button__little button--green" role="button"><img src="/assets/images/attach-files/icon-1.svg" alt="">Прикрепить ещё</span><input class="attach__input" type="file" name="files[]" accept="image/*"/></label><div class="attach__name"></div><div class="attach__delete"><a title="Удалить"><img src="/assets/images/tasks/delete.svg" alt="Удалить"></a></div></div>'; // разметка нового поля
    ...
    attach.on('change', '.attach__input', function(e) {
      ...
      if (fileName) { // если имя файла не пустое
        ...
        if (fields == fieldsAttached) { // если полей меньше 10 и кол-во полей равно
          item.after($(newButtonAttachFiles)); // добавляем новое поле
          fields++;
        }
        ...
        if (fields == fieldsAttached + 1) {
          ...
          item.replaceWith($(newButtonAttachFiles)); // заменяем поле на "чистое"
        }
        fieldsAttached--;
    ...
    attach.on('click', '.attach__delete', function() {
      ...
      fieldsAttached--;
      if (fields == 1) { // если поле осталось одно
        attach.find('.attach__up').html('<img src="/assets/images/attach-files/icon-1.svg" alt="">Прикрепить файл'); // меняем текст
      }
    });
  });
})
