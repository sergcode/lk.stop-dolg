...
function warningClosingQuestionnaire() {
	...
	const beforeUnloadListener = (event) => {
		...
		return event.returnValue = "Уважаемый клиент, не вся информация в анкете заполнена, вы уверены что хотите прервать анкетирование?";
	}
	...
	const warningPopupCall = (href) => {
      ...
      <p>Уважаемый клиент, не вся информация в анкете заполнена,<br/> вы уверены, что хотите прервать анкетирование?</p>
		...
		elemLevelOne.forEach(function (elemLevelTwo) {
			['input', 'change'].forEach(evt => {
				elemLevelTwo.addEventListener(evt, (event) => {
					...
					menuBtn.forEach((elemArrBtn) => {
						[elemArrBtn, logoHeader, buttonLogout].forEach((elemBtn) => {
					...
					document.querySelector('.questionnaire__return').addEventListener('click', () => {
						removeEventListener('beforeunload', beforeUnloadListener, {capture: true});
					});
				});
			});
...
function validationSendQuestionnaire() {
	...
	const closeQuestionnaireAndOpenSuccessfully = () => {
		let questionnaireElements = document.querySelectorAll('.questionnaire__form_input, .switches__checkbox, .questionnaire__form_select, .questionnaire__form_textarea, .button[name="createBlock"], .button[name="deleteBlock"]'),
		...
		questionnaireReturn.style.display = 'flex';
	...
	const finishWarningPopupCallSend = () => {
		warningPopupWrapper.innerHTML = `
              <p>Уважаемый клиент, не вся информация на последней странице заполнена,<br/>вы уверены, что хотите отправить анкету?</p>
              <div class="warning-popup__button">
                <button class="button button__middle button__white" name="sendQuestionnaireForcibly" role="button">
                  Да, отправить анкету
                </button>
                <button class="button button__middle button__white" name="continueFilling" role="button">
                  Продолжить заполнение этой страницы
		...
		sendQuestionnaire.addEventListener('click', () => {
			...
			if (owlItemActive.classList.contains('active')) {
				...
				questionnaireInputs.forEach((elemInput) => {
					if (valid && elemInput.value == '' || valid && elemInput.value == ' ') {
