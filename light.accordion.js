...
function createModalSendFileActiveTask(successfullyUnsuccessfullyClass, title, subtitle, img) {
  let toggleModalBg = document.querySelector('.toggle-modal-bg');
  const toggleModal = document.createElement('div');
  toggleModal.classList.add('toggle-modal', successfullyUnsuccessfullyClass);
  ...
  toggleModal.innerHTML = `
      ...
      <div class="toggle-modal__body toggle-modal__body_white toggle-modal__body_pt-sm-50">
      	<article role="article">
      		<figure>
      			<img src="/assets/images/icons/${img}" alt="">
      			<figcaption class="toggle-modal__text toggle-modal__title">${title}</figcaption>
        		<figcaption class="toggle-modal__text toggle-modal__subtitle toggle-modal__subtitle_mb-0">${subtitle}</figcaption>
					</figure>
				</article>
      </div>
    </div>
...
const activeAndCloseModal = {
  ...
  activeModalSendFileActiveTask: function () {
    createModalSendFileActiveTask('toggle-modal__successfully', 'Файлы успешно отправлены!', 'Задача передана юристу на проверку.', 'send-success-email.svg');
  ...
  activeModalErrorSendFileActiveTask: function () {
    createModalSendFileActiveTask('toggle-modal__unsuccessfully', 'Ошибка при отправке файлов!', 'Задача не передана юристу на проверку.', 'send-error-email.svg');
...
function formatFiles(extension, elem) {
  switch (extension) {
    ...
    case 'tif':
    case 'bmp':
      elem.classList.add('attach__file_wrapper__purple');
      break;

    case 'pdf':
      elem.classList.add('attach__file_wrapper__red');
      break;
    ...
    case 'odt':
    case 'txt':
      elem.classList.add('attach__file_wrapper__dark-blue');
      break;

    case 'xls':
    case 'xlsx':
      elem.classList.add('attach__file_wrapper__green');
      break;

    case 'ppt':
    case 'pptx':
      elem.classList.add('attach__file_wrapper__orange');
      break;
    ...
    case 'arj':
    case 'jar':
      elem.classList.add('attach__file_wrapper__gold');
      break;

    default:
      elem.classList.add('attach__file_wrapper__neutral');
      break;
  }
}
...
function attachFileCopyExtension() {
  const attachedFiles = document.querySelectorAll('.task__accordion_item--buttons-wrapper ul li a, .task__accordion_item--buttons .task__accordion_item--list-files__downloads li a');
...
/** todo Манипуляции над Чекбоксами файлов от юриста для скачивания */

function listFilesDownloads() {
  const listFilesDownloads = document.querySelectorAll('.task__accordion_item--list-files__downloads'); // ul

  for (let parent of listFilesDownloads) {
    const checkedAllCheckbox = parent.querySelector('.task__accordion_item--list-files__checkbox_checked-all input[type="checkbox"]'),
          listFilesWrapper = parent.querySelectorAll('.task__accordion_item--list-files__wrapper'),
          // li
    buttonsWrapper = parent.nextElementSibling,
          attachButtons = buttonsWrapper.querySelectorAll('.attach__button');

    for (let btn of attachButtons) {
      const btnSpan = btn.querySelector('span');

      if (btn.getAttribute('name') === 'buttonDownloadAllFiles') {
        if (listFilesWrapper.length === 1) {
          btnSpan.textContent = 'Скачать файл';
        } else {
          btnSpan.textContent = 'Скачать файлы';
        }
        /** todo Скачать все файлы */


        btn.addEventListener('click', e => {
          e.preventDefault();
          download(listFilesWrapper).then();
        });
      }
    }

    for (let parentWrap of listFilesWrapper) {
      const listFilesCheckbox = parentWrap.querySelector('.task__accordion_item--list-files__checkbox'),
            // div
      checkbox = listFilesCheckbox.querySelector('input[type="checkbox"]'),
            fileDownload = parentWrap.querySelector('a[download]');
      /** todo Чекбокс Выбрать все файлов */

      checkedAllCheckbox.addEventListener('change', function (e) {
        e.preventDefault();

        for (let btn of attachButtons) {
          if (this.checked) {
            checkbox.checked = false;
            checkbox.checked = true; // fileDownload.classList.add('included');
            // setTimeout(() => fileDownload.classList.remove('included'), 1500);

            if (btn.classList.contains('disabled')) btn.classList.remove('disabled');
            if (btn.getAttribute('name') === 'buttonDownloadAllFiles' && btn.hasAttribute('disabled')) btn.disabled = false;
          } else {
            checkbox.checked = false; // fileDownload.classList.remove('included');

            if (!btn.classList.contains('disabled')) btn.classList.add('disabled');
            if (btn.getAttribute('name') === 'buttonDownloadAllFiles' && !btn.hasAttribute('disabled')) btn.disabled = true;
          }
        }
      });
      /** todo Чекбоксы файлов */

      checkbox.addEventListener('change', function (e) {
        e.preventDefault();
        const thisCheckbox = this,
              checkboxChecked = parent.querySelectorAll('.task__accordion_item--list-files__wrapper input[type="checkbox"]:checked'),
              checkboxUnChecked = parent.querySelectorAll('.task__accordion_item--list-files__wrapper input[type="checkbox"]');

        if (checkedAllCheckbox.checked && checkboxUnChecked.length > checkboxChecked.length) {
          checkedAllCheckbox.checked = false;
        }
        /*if (thisCheckbox.checked) {
        	fileDownload.classList.add('included');
        	setTimeout(() => fileDownload.classList.remove('included'), 1500);
        		} else {
        	fileDownload.classList.remove('included');
        }*/
...
/** todo Одновременное скачивание нескольких файлов */


async function download(listFilesWrapper) {
  for await (let li of listFilesWrapper) {
    const checkbox = li.querySelector('input[type="checkbox"]'),
          fileDownload = li.querySelector('a[download]');
    await delay();

    if (checkbox.checked) {
      const file = await fetch(fileDownload.getAttribute('href')),
            fileBlog = await file.blob(),
            fileURL = URL.createObjectURL(fileBlog);
      const anchor = document.createElement('a');
      anchor.href = fileURL;
      anchor.download = fileDownload.getAttribute('download');
  ...
  moreInfoParent.forEach(parent => {
    ...
    function accordionsItemBtnIsOpen(accordionsItemBtn) {
      ...
      if (mediaQueryTask.matches && onCheckTaskTrue) {
        ...
        } else if (!accordionsItemBtnIsOpenTrue && !wrapperText.lastElementChild.classList.contains('task__accordion_item--info-in-processing')) {
          // Расположить "Инф. в обработке" под заголовком задачи
    ...
    accordionsItemBtn.forEach(itemBtn => {
      ...
      if (mqt.matches) {
        if (elemParent.matches('[name="onCheckTask"]')) {
          ...
          } else if (!itemBtnClassIsOpenTrue && !wrapperTextChildDeadline) {
            // Если у дочернего аккордеона нет класса is-open и у дочернего аккордеона нет в заголовке "Инф-я в обработке"
