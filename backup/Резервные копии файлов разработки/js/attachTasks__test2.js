...
// 				docs[[+id]] = document.querySelector('#sendImg[[+id]]');
//
// 	if (docs[[+id]]) {
// 		docs[[+id]].addEventListener('click', function(e) {
// 			e.preventDefault();
//
// 			let input[[+id]] = document.querySelector('#file[[+id]]');
//
// 			input[[+id]].onchange = e => {
// 				let fileList[[+id]] = e.target.files,
// 						fd[[+id]] = new FormData();
//
// 				fd[[+id]].append("action", "filesUploaded");
// 				fd[[+id]].append("task", "[[+id]]");
//
// 				for (let x=0; x < fileList[[+id]].length; x++) {
// 					fd[[+id]].append('file['+ x +']', fileList[[+id]].item(x));
// 				}
//
// 				let xhr[[+id]] = new XMLHttpRequest();
//
// 				xhr[[+id]].upload.onprogress = (event) => {
// 					document.getElementById('progressBar[[+id]]').hidden = false;
// 					if (event.total) {
// 						const percent[[+id]] = Math.round((event.loaded / event.total) * 100);
// 						document.getElementById('loadFile[[+id]]').value = percent[[+id]];
// 					}
// 				};
//
// 			};
