...
window.addEventListener('DOMContentLoaded', eventDOM => {
  ...
  function showPopupSBP() {
    const body = document.body,
          ...
          formSbpContainer = document.getElementById('sbpOplata'),
          header = document.querySelector('.header'),
          backTop = document.querySelector('.backTop');
    body.addEventListener('click', e => {
      const target = e.target,
            b24widget = document.querySelector('.b24-widget-button-position-bottom-right');
      ...
      if (target && target.hasAttribute('data-payment-id')) {
        ...
        if (document.body.scrollHeight !== window.innerHeight) {
          ...
          if (mediaQueryDesktop.matches) {
            header.classList.add('header__modal-show');
            backTop.classList.add('backTop__modal-show');

            if (b24widget) {
              b24widget.classList.add('b24-widget-button__modal-show');
            }
          }
        }
      ...
      if (target && (target.classList.contains('popup') || target.classList.contains('popup__close'))) {
        ...
        if (document.body.classList.contains('overflow-hidden')) {
          ...
          if (mediaQueryDesktop.matches) {
            header.classList.remove('header__modal-show');
            backTop.classList.remove('backTop__modal-show');

            if (b24widget) {
              b24widget.classList.remove('b24-widget-button__modal-show');
            }
          }
        }
  ...
  /** todo Копирует url из атрибута data-pay-link и подставляет в кнопку Перейти к оплате */
...
const accord = Vue.component('Expander', {
  ...
  props: {
    ...
    animation: {
      type: String,
      default: 'rightToLeft' // validator: prop => ['leftToRight', 'bounceIn', 'bottomToTop'].includes(prop)
