...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let parentModal = document.querySelector('.toggle-modal');
  ...
  formSubmit.addEventListener('click', function () {
    if (formTextarea.value.length >= 5) {
      ...
      parentModal.classList.add('remove'); // Скрываем модалку с отзывом и

      setTimeout(() => {
        parentModal.classList.remove('active');
        parentModal.classList.remove('remove');
  ...
  overlay.addEventListener('click', function () {
    parentModal.classList.add("remove");
    overlay.classList.add("remove");
    setTimeout(() => {
      parentModal.classList.remove('active');
      parentModal.classList.remove("remove");
...
//# sourceMappingURL=toggle.modal.js.map
