...
window.addEventListener('DOMContentLoaded', eventDOM => {
  ...
  function showPopupSBP() {
    const body = document.body,
          ...
          popupButtonPay = popupBody.querySelectorAll('.popup__button_pay');
    body.addEventListener('click', e => {
      ...
      if (target && target.hasAttribute('data-payment-id')) {
        ...
        const linkSbp = target.getAttribute('data-page_blank'),
              ...
              company_id = target.getAttribute('data-company');
        ...
        popupButtonPay.forEach(btnPay => {
          if (btnPay.getAttribute('id') === 'sbpOplata') {
            btnPay.setAttribute('href', `${linkSbp}`);
            if(parseInt(company_id)!=3 && parseInt(company_id)!=5){
          ...
          if (btnPay.getAttribute('id') === 'payByBankCard') {
            btnPay.setAttribute('href', `${linkBankCard}`);
          }
        });
      }
  ...
  showPopupSBP(); // copyLinkRadioForBtn();
