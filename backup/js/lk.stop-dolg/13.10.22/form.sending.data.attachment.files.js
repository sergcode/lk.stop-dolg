...
window.addEventListener('DOMContentLoaded', () => {
  ...
  let filesArr = [],
      // Массив для отправки файлов на почту
  newFiles = [],
      // Вспомогательный массив для первого массива, нужен чтобы найти дубликаты файлов
  duplicateFiles = [],
      // Массив дубликатов файлов
  deleteFiles = [],
      // Массив удаленных при замене дубликатов файлов. Нужен, чтобы восстановить первоначальные значения в первом массиве.
  deleteNewFiles = [],
      // Массив удаленных при пропуске дубликатов файлов. Нужен, чтобы восстановить первоначальные значения во втором массиве.
  ...
  toggleModalBg.classList.add('toggle-modal-bg', 'toggle-modal-bg__warning');
  ...
  for (let form of forms) {
    ...
    if (form.classList.contains('ajax-save')) {
      const closeButtons = form.querySelectorAll('.js-modal-close'),
            checkedStar = form.querySelector('input[name="rating"]'),
      ...
      inputs.forEach((input, i) => {
        ...
        input.addEventListener('change', function (e) {
          ...
          const thisInput = this,
                ...
                thisToggleModal = thisInput.closest('.toggle-modal'),
          ...
          for (let file of newFiles) {
            if (filesArr.some(({
              ...
              if (duplicateFiles.length > 1) {
                toggleModalWarningSetting(`В прикрепленных файлах есть файлы (${duplicateFiles.length}) с такими же именами`, 'Заменить файлы', 'Пропустить файлы', false, true);
              } else {
                toggleModalWarningFileName.textContent = `${file.name}`;
                toggleModalWarningSetting('В прикрепленных файлах есть файл с таким же именем', 'Заменить файл', 'Пропустить файл', true, true);
              }
          ...
          if (mediaQueryTaskSmart.matches && wrapperBtnDeleteFiles.classList.contains('show')) {
          ...
          }); // screenSize(thisToggleModal);
        });
        /*				document.body.addEventListener('click', (e) => {
        					const thisBtnDeleteFiles = e.target;
        
        					if (thisBtnDeleteFiles && thisBtnDeleteFiles.matches('.attach__delete-files')) {
        						e.preventDefault();
        						deleteAttachedFiles(wrapperCreatedFiles, input, thisBtnDeleteFiles);
        						screenSize(thisBtnDeleteFiles.closest('.toggle-modal'));
        					}
        				});*/
        ...
        wrapperBtnDeleteFiles.addEventListener('click', function (e) {
          ...
          screenSize(this.closest('.toggle-modal'));
        });
      });
            ...
            $.ajax({
              ...
              success: data => {
                if (JSON.parse(data).result === 'error') {
                  ...
                  setTimeout(() => {
                    // После того как модалку с успешно отправленным отзывом закрыта, очистить textarea через 1cек.
    ...
    closeElement.forEach(item => {
      item.addEventListener('click', function () {
        const parentModalClose = this.closest('.toggle-modal'),
              modalBox = parentModalClose.querySelector('.toggle-modal__box');
        ...
        if (parentModalClose.classList.contains('active')) {
          ...
          if (parentModalClose.classList.contains('toggle-modal__animation-hide')) {
            parentModalClose.classList.remove('toggle-modal__animation-hide');
          }

          if (parentModalClose.classList.contains('toggle-modal__scroll')) {
            parentModalClose.classList.remove('toggle-modal__scroll');
  ...
  function closeModalFormOverlay(modal, formTextarea, formTooltip) {
    overlay.addEventListener('click', e => {
      ...
      if (modal.classList.contains('active')) {
        ...
        if (modal.classList.contains('toggle-modal__animation-hide')) {
          modal.classList.remove('toggle-modal__animation-hide');
        }
    ...
    if (newFiles.length) {
      for (let file of newFiles) {
        const fileName = file.name,
              ifFileNamesAreEqual = filesArr.some(({
          name
        }) => name === fileName),
              ifFileNamesAreUnequal = filesArr.some(({
          name
        }) => name !== fileName);

        if (event.target.matches('[data-action="requestForEachFile"]')) {
          toggleModalWarning.classList.add('requestForEachFile');
          toggleModalWarningSetting('В прикрепленных файлах есть файл с таким же именем', 'Заменить файл', 'Пропустить файл', true, false);
        }
        ...
        if (toggleModalWarning.classList.contains('requestForEachFile')) {
          ...
          if (event.target.matches('[data-action="replaceFile"]')) {
            ...
            if (filesArr.some(({
              name
            }) => name === modalFileName) && filesArr[filesArr.findIndex(el => el.name === modalFileName)]) {
            ...
            if (ifFileNamesAreEqual && filesArr[filesArr.findIndex(el => el.name === fileName)]) {
              ...
              changingFileName(filesArr[filesArr.findIndex(el => el.name === fileName)].name); // toggleModalWarningFileName.textContent = `${filesArr[filesArr.findIndex(el => el.name === fileName)].name}`;
          ...
          if (event.target.matches('[data-action="skipFile"]')) {
            ...
            if (newFiles.some(({
              name
            }) => name === modalFileName) && newFiles[newFiles.findIndex(el => el.name === modalFileName)]) {
            ...
            if (ifFileNamesAreEqual && newFiles[newFiles.findIndex(el => el.name === filesArr[filesArr.findIndex(el => el.name === fileName)].name)]) {
              toggleModalWarningNumbFile.textContent = `${Math.floor(num + 1)} / ${duplicateFiles.length}`;
              changingFileName(fileName); // toggleModalWarningFileName.textContent = `${fileName}`;
  ...
  function cleaningClosing(toggleModalWarning) {
    if (toggleModalWarningNumbFile.textContent && toggleModalWarningFileName.textContent) {
      toggleModalWarningNumbFile.textContent = '';
      toggleModalWarningFileName.textContent = '';
    }
  ...
  function addFilesToArray(file, parent, trueFalse) {
    const fileName = file.name,
          filenameWithoutExtension = fileName.beforeLastIndex("."),
  ...
  function addScroll(parent) {
    if (mediaQueryTaskSmart.matches) {
      parent.childElementCount > 2 ? parent.classList.add(wrapperCreatedFilesScroll) : parent.classList.remove(wrapperCreatedFilesScroll);
    }
  }
  ...
  function handleOrientationChangeSmartphone(mqts) {
    forms.forEach(form => {
      ...
      if (toggleModal && toggleModal.classList.contains('active')) {
        if (form.classList.contains('ajax-save')) {
          const modalBox = toggleModal.querySelector('.toggle-modal__box'),
                wrapperCreatedFiles = form.querySelector('.attach__wrapper-created-files'),
                wrapperBtnDeleteFiles = form.querySelector('.attach__delete-files');

          if (mqts.matches) {
            if (wrapperCreatedFiles.childElementCount > 2 && !wrapperCreatedFiles.classList.contains(wrapperCreatedFilesScroll)) {
              wrapperCreatedFiles.classList.add(wrapperCreatedFilesScroll);
            }

            wrapperBtnDeleteFiles.classList.remove('show');
            toggleModal.classList.add('toggle-modal__animation-hide');
            toggleModal.classList.remove('toggle-modal__scroll');
            modalBox.removeAttribute('style');
          } else {
            const innerHeight = window.innerHeight;

            if (wrapperCreatedFiles.classList.contains(wrapperCreatedFilesScroll)) {
            ...
            toggleModal.classList.remove('toggle-modal__animation-hide');
            toggleModal.classList.add('toggle-modal__scroll');
  ...
  function screenSize(modal, mqts) {
    const innerHeight = window.innerHeight,
          modalBox = modal.querySelector('.toggle-modal__box');

    if (mqts.matches && modalBox.scrollHeight < innerHeight) {
  ...
  function deleteAttachedFile(wrapperCreatedFiles) {
    ...
    for (let [i, btn] of btnDel.entries()) {
      btn.onclick = function (e) {
        e.preventDefault();
        const parentToggleModal = this.closest('.toggle-modal'),
              parentOfBtnDelete = this.closest('.attach__file'),
  ...
  document.body.addEventListener('click', e => {
    if (e.target.matches('[class="toggle-modal__close js-modal-close"]')) {
      e.preventDefault();
      console.log(filesArr); // console.log(deleteFiles + ' - Замена');
