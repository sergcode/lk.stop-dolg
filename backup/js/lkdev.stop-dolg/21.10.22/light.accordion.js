...
function sendFilesActiveTask() {
  ...
  accordionsParentBtn.forEach(parentBtn => {
    let accordionsParentBtnSpan = parentBtn.querySelectorAll('span'),
        ...
        accordionsItemBtn = accordionItemMoreInfo.querySelectorAll('.task__accordion-item-btn'),
        // Находим все задачи ItemBtn;
            ...
            xhr.addEventListener('load', () => {
              if (xhr.status === 200) {
                ...
                moreInfoAttachNameFile.forEach(attachName => {
                  let nameFile = attachName.textContent,
                      // Имя прикрепленного файла
                  extensionFile = attachName.parentElement.querySelector('p').textContent,
